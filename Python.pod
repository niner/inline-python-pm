...
This is a BETA quality release of Inline::Python. It is fairly feature complete
and runs stable with no known crasher bugs. Further testing and expanded support
for other operating systems and platforms will be a focus for future releases.
...
Inline::Python is known to leak memory. Version 0.24 fixes most of these leaks,
but it's still not completely clean and I have no idea where the remaining
leaks would be.

There my also be segfaults on errors happening in Python space. If these happen
for you, try catching exceptions with try/except, before they reach Perl.
...
openSUSE at least from 10.3 to 11.2 and on Solaris.  Previous versions of
