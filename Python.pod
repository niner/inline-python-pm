...
and runs stable with only known crasher bugs on throwing unhandled exceptions in
Python code. Further testing and expanded support for other operating systems
...
There are also quite a few segfaults on errors happening in Python space. If
these happen for you, try catching exceptions with try/except, before they
reach Perl.
